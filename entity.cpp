...
void Entity::updateMovement(){
	...
	//cout << "Move To: (" << collisionBox.x << "," << collisionBox.y << ")" << endl;
	...
	//cout << "Slide Angle: " << slideAngle << endl;
	//sliding around!
	if (slideAmount > 0){
...
void Entity::updateCollisionBox(){
	...
	collisionBox.y = y + collisionBoxYOffset; //collisionBoxYOffset is negative
...
void Entity::updateCollisions(){
	if (active && collideWithSolids && (moveSpeed > 0 || slideAmount > 0)){
		...
		if (collisions.size() > 0){
			...
			float collisonMoveX = sampleBox.x, collisonMoveY = sampleBox.y;
			...
			while (!foundCollision){
				...
				for (auto entity = collisions.begin(); entity != collisions.end(); entity++){
					if (SDL_IntersectRect(&sampleBox, &(*entity)->collisionBox, &intersection))
					{
						...
						//cout << "Setted The Slide Angle: " << slideAngle << endl;
						...
						if (intersection.w < intersection.h){
							if (lastCollisionBox.x + lastCollisionBox.w / 2 < (*entity)->collisionBox.x + (*entity)->collisionBox.w / 2)
								sampleBox.x -= intersection.w; //started on left, so move left out of collision0
							else
				...
				if (distanceBetweenTwoRects(sampleBox, collisionBox) > boxTravelSize){
					...
					//fix for the float values below 1, e.g. 0.71 being truncated to sampleBox.x as that is an int
					collisonMoveX += xMove;
					collisonMoveY += yMove;
					sampleBox.x = collisonMoveX;
					sampleBox.y = collisonMoveY;
				}
				else{
			...
			if (foundCollision){
				...
				//cout << "Setted The Slide Amount: " << slideAmount << endl;
